...
public class GamePanel extends JPanel implements KeyListener {
    ...
    public GamePanel(){
        player = new Player(475, 650, this); // player spawn location
        makeWalls();
        makeCoins();
    ...
    public void makeWalls() {
        ...
        while (n < 55) {
            ...
            for (int j = 0; j < 3; j++) {   // walls of length 3 blocks
                walls.add(new Wall((j + n1) * 25, n2 * 25, 25, 25, Color.green));
                if (j < 0) {    // walls of length 2 blocks
                    walls.add(new Wall((j + n3) * 25, n4 * 25, 25, 25, Color.green));
                }
                if (j < 1) {    // walls of length 1 block
